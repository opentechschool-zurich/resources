...
- Each participant gets a different task (from the simplest to the most complicated):
  - add the substraction
  - add multiplication and division
  - add `print_calculation(a, op, b)` that prints the calculation and the result:  
    `print_calculation(3, '+', 2)` prints `3 + 2 = 5`.
  - add user input (until an empty line is entered)
  - add `calculate_list([3, '+', 2, '-', 1])` that uses `calculation(a, op, b)` and does not respect the operators priorities.
- In a first round:
